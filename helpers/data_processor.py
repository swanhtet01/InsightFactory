...
def find_column_by_pattern(df, patterns, strict=False, min_non_na=1):
    """Find column name containing any of the patterns. If not found, fallback to first non-empty, non-index column (unless strict)."""
    for col in df.columns:
        ...
        for pattern in patterns:
            ...
            if clean_pattern in clean_col:
                return col
    # Fallback: return first non-empty, non-index column
    if not strict:
        for col in df.columns:
            if not str(col).lower().startswith('unnamed') and df[col].notna().sum() >= min_non_na:
                logger.warning(f"Falling back to column '{col}' for patterns {patterns}")
                return col
        logger.warning(f"Could not find column matching patterns: {patterns}, and no fallback found.")
        logger.info(f"Available columns: {list(df.columns)}")
        return df.columns[0] if len(df.columns) > 0 else None
    else:
        logger.warning(f"Strict mode: Could not find column matching patterns: {patterns}")
        logger.info(f"Available columns: {list(df.columns)}")
        return None
...
def get_latest_production_data():
    ...
    all_data = []
    used_files = {}
    # Find latest file for each type
    files = [f for f in os.listdir(data_dir) if f.endswith('.xlsx')]
    def latest_file(pattern):
        matches = [f for f in files if pattern in f]
        if not matches:
            return None
        matches = sorted(matches, key=lambda x: os.path.getmtime(os.path.join(data_dir, x)), reverse=True)
        return matches[0]

    # Daily
    daily = latest_file('Daily')
    if daily:
        file_path = os.path.join(data_dir, daily)
        daily_data = process_daily_data(file_path, os.path.join(data_dir, 'images/2025/daily'))
        if not daily_data.empty:
            all_data.append(daily_data)
            used_files['daily'] = daily

    # Weekly
    weekly = latest_file('Weekly')
    if weekly:
        file_path = os.path.join(data_dir, weekly)
        weekly_data = process_weekly_data(file_path, os.path.join(data_dir, 'images/2025/weekly'))
        if not weekly_data.empty:
            all_data.append(weekly_data)
            used_files['weekly'] = weekly

    # Monthly
    monthly = latest_file('Tyre PD')
    if monthly:
        file_path = os.path.join(data_dir, monthly)
        monthly_data = process_monthly_data(file_path, os.path.join(data_dir, 'images/2025/monthly'))
        if not monthly_data.empty:
            all_data.append(monthly_data)
            used_files['monthly'] = monthly

    # Annual
    annual = latest_file('year')
    if annual:
        file_path = os.path.join(data_dir, annual)
    ...
    logger.info(f"Used files for data: {used_files}")
    return combined_df
